...
const chatHandler: ChatHandler = async (question: string) => {
  ...
  return {
    ...
    answerCallBack: async (answerText: string) => {
      chatHistroy.push(new HumanMessage(contextualizedQuestion || question));
      chatHistroy.push(new AIMessage(answerText));
    },
  };
};
